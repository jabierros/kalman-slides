...
const processIMG = (img) => {
  ...
  fetch(svgURL)
    ...
    .then((svgContent) => {    
      ...
      // Fix Obsidian-specific URL formats
      ...
      // Rewrite standard Excalidraw file links to point to the web-friendly SVG versions
      // Rewrite standard Excalidraw file links to use our dynamic index.html viewer
      svgContainer.querySelectorAll("a").forEach(el => {
        let currentHref = el.getAttribute("href");
        
        if (currentHref) {
           let targetSVG = "";
           
           // If the link already points to an SVG file
           if (currentHref.endsWith(".svg")) {
               targetSVG = currentHref;
           }
           // If the link ends in .excalidraw
           else if (currentHref.endsWith(".excalidraw")) {
               targetSVG = currentHref + ".svg";
           } 
           // If the link ends in .excalidraw.md
           else if (currentHref.endsWith(".excalidraw.md")) {
               targetSVG = currentHref.replace(".excalidraw.md", ".excalidraw.svg");
           }

           // If we found a valid slide link, route it through index.html
           if (targetSVG !== "") {
               el.setAttribute("href", "index.html?file=" + encodeURIComponent(targetSVG));
           }
        }
      });

      // Swap the static image for the interactive SVG
